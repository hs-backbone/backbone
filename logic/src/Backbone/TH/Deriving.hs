...
-- | This class provides an artificial hierarchy that defines all the classes that a "well behaved" 
-- data type should implement. All newtypes will derive them automatically.
...
listSuperClasses className = do
  ...
  where
    ...
    go var (AppT (ConT name) (VarT var')) =
        if var == var' then listSuperClasses name else return [] -- class depends on another type tested elsewhere
    go _ _ = return []

tuple2list :: Type -> [Type]
tuple2list (AppT (AppT (TupleT 2          ) t1) t2) = [t1, t2]
tuple2list (AppT (AppT (AppT (TupleT 3) t1) t2) t3) = [t1, t2, t3]
tuple2list (AppT (AppT (AppT (AppT (TupleT 4) t1) t2) t3) t4) =
    [t1, t2, t3, t4]
tuple2list (AppT (AppT (AppT (AppT (AppT (TupleT 5) t1) t2) t3) t4) t5) =
    [t1, t2, t3, t4, t5]
tuple2list t = [t]
...
deriveHierarchyFiltered typename classnameL filterL = do
    classL <-
        fmap concat $ mapM listSuperClasses $ mkName "BasicType" : classnameL
    instanceL <-
        mapM (deriveSingleInstance typename)
        $ filter (\x -> not (x `elem` filterL))
        $ nub classL
...
deriveSingleInstance typename classname = deriveSingleInstance typename classname [
...
-- | Given a single newtype and single class, constructs newtype instances
doDeriveSingleInstance :: Name -> Name -> [String] -> Q [Dec]
doDeriveSingleInstance typename classname typeFamilies =
    ...
    typefamiliesD <- deriveTypefamilies
        typeFamilies
        typename

    classinfo <- reify classname
    liftM (typefamiliesD ++) $ case classinfo of
        ...
        ClassI (ClassD _ _ _ _ _) [InstanceD _ _ (VarT _) _] ->
            return []
        ClassI (ClassD _ _ _ _ _) [InstanceD _ _ (AppT (ConT _) (VarT _)) _]
            -> return []
                    ...
                    funcL <-
                        forM (filter notDefaultSigD decs) $ \dec -> do
                            ...
                            return
                                ...
                                , PragmaD
                                    $ InlineP f Inline FunLike AllPhases
                                ]

                    return
                        [ InstanceD
                                Nothing
                                ( AppT (ConT classname) typeapp
                                : map (subVarT varname typeapp) ctx
                                )
                                (AppT (ConT classname) $ apply2varlist
                                    (ConT typename)
                                    typekind
                                )
                                (concat funcL)
...
subVarT varname vartype t = go t
  where
    go (VarT e    ) = if e == varname then vartype else VarT e
    go (ConT e    ) = ConT e
...
returnType2newtypeApplicator conname varname t exp' = do
  ...
  where
    ...
    go (VarT   v        ) = if v == varname then return $ ConE conname else id'
    go (ConT   _        ) = id'
    ...
    go (AppT (ConT c) t2) = do
        ...
        case info of
            ...
            TyConI (NewtypeD _ _ _ _ _ _) ->
                liftM (AppE (VarE $ mkName "helper_liftM")) $ go t2
    ...
    go (AppT (AppT (TupleT 2) t1) t2) = do
        ...
        return $ LamE
            ...
            (TupE [AppE e1 (VarE $ mkName "v1"), AppE e2 (VarE $ mkName "v2")])
...
substituteNewtype conname varname _ = go
  where
    go (VarT v) =
        if varname == v then AppT (ConT conname) (VarT varname) else VarT v
    go (AppT t1 t2) = AppT (go t1) (go t2)
    go (ConT t    ) = ConT t

typeL2patL :: Name -> Name -> [Type] -> [Pat]
typeL2patL conname varname xs = map go
    $ zip (map (\a -> mkName [a]) ['a' ..]) xs
  where
    go :: (Name, Type) -> Pat
    go (newvar, VarT v) =
        if v == varname then ConP conname [VarP newvar] else VarP newvar
    go (newvar, AppT (AppT (ConT c) _) _) = VarP newvar
...
fromPreludeEq qt = do
    t <- qt
    return
        [ TySynInstD (mkName "Logic") (TySynEqn [t] (ConT $ mkName "Bool"))
        , InstanceD
            Nothing
            []
